...
function draw() {
  ...
  // Desenhar a mesa de ping pong
  stroke(255);
  strokeWeight(2);
  line(width / 2, 0, width / 2, height);

  fill(255);
  rect(width - paddleWidth - 10, paddle1Y, paddleWidth, paddleHeight);
  rect(10, paddle2Y, paddleWidth, paddleHeight);
  ballX += ballSpeedX;
  ballY += ballSpeedY;

  if (
    ballX + ballSize / 2 >= width - paddleWidth - 10 &&
    ballY + ballSize / 2 >= paddle1Y &&
    ballY - ballSize / 2 <= paddle1Y + paddleHeight
  ) {
    if (ballSpeedX > 0) {
      ballSpeedX *= -speedMultiplier;
      ballSpeedY = calculateBallSpeedY(paddle1Y, ballY);
    }
  }
  ...
  if (
    ...
    if (ballSpeedX < 0) {
      ballSpeedX *= -speedMultiplier;
      ballSpeedY = calculateBallSpeedY(paddle2Y, ballY);
    }
  }
...
function calculateBallSpeedY(paddleY, ballY) {
  let relativeIntersectY = paddleY + paddleHeight / 2 - ballY;
  return (relativeIntersectY / (paddleHeight / 2)) * 5;
}